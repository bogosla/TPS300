...
void TP_GetFontInfo( ST_FONT *AsciiFont,ST_FONT *CfontFont)
{
    ...
    for(tempi=0; tempi<FontCnt; tempi++)
    {
        ...
        {
            if(FontBuf[tempi].Width != 0 && FontBuf[tempi].Height != 0)//��֤������Ч��
            {
                      ...
                      char* (*strAddrBuf)[], int32 MaxLineCount )
{
    ...
    while (Index < strlen((char *)DisplayData))
    {
        ...
        else if (LineLen > MaxCharInRow) //�����ֻ��������ʾ�����һ����ռ2���ַ����ȵ����
        {
            (*strAddrBuf)[ LineCount ] = (char*)StartPoint;
            LineCount++;
 ...
 * @author linhx
 *
 * @param showIMIcon TRUE��show icon <BR>
 *                   FALSE��don't show the icon
 *
    ...
    while (TRUE)
    {
        ...
        while(1)
        {
            ...
            if (0 != TP_Kbhit())
            {
                ...
                } else if (userKey == TP_KEY_FUNC) {
                    // TP_ScrRestore(0);
                    if (callback != NULL)
                        callback(data, size, start_date, end_date);
                    // TP_ScrRestore(1);
                                 ...
                                 uint8 *icon)
{
    ...
    TP_ScrCls();
    SetScrOption();
    TP_ScrFontSet(CFONT);//listbox�õ���CFONT����
...
Boolean TP_DateInput( TP_DisplayInfo *displayInfo, TP_Date *pDate )
{
    ...
    TP_ScrSpaceGet(&charSpace, &lineSpace);
    DrawTitileMenu(displayInfo, 2, 2, MAX_SCREEN_WIDTH - 1, fontHeight);
    // DrawFootMenu( displayInfo, 1, MAX_SCREEN_HEIGHT - fontHeight, MAX_SCREEN_WIDTH - 1, MAX_SCREEN_HEIGHT - 1);
    ...
    ***************************************************************/
    {
        ...
        drawTop = fontHeight + (MAX_SCREEN_HEIGHT - fontHeight*3)/2 ;//������ʾ
