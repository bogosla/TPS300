...
void SwitchCardTest(void)
{
    uint8 keyCode;
    uint8 currentSlot;
    uint32 switchTime = 0;
    uint32 networkTime = 0;
    uint32 gprsTime = 0;
    char apnStr[21];
    char userNameStr[21];
    char passwordStr[21];
    ...
    TP_ScrGotoxyEx(0, 0);
    TP_LcdPrintf("Card Switch Test");
    currentSlot = TP_MobileCardGetActiveCardSlot();

    TP_ScrGotoxyEx(0, 13);
    if (currentSlot == 0)
    {
        TP_LcdPrintf("Switch to SIM2 card?");
    }
    else
    {
        TP_LcdPrintf("Switch to SIM1 card?");
    }

    TP_ScrGotoxyEx(0, 52);
    TP_LcdPrintf("OK");
    TP_ScrGotoxyEx(92, 52);
    TP_LcdPrintf("Cancel");
    ...
    TP_ScrGotoxyEx(0, 13);
    TP_LcdPrintf("Card switching ...");

    TP_BanIncomingCall(TRUE);
    TP_TimerSet(0, 60 * 1000);
    TP_MobileCardSetActiveCardSlot(currentSlot);
    while (TP_GetMobileCardStatus() == TP_MOBILECARDSWITCHING)
    {
        TP_MessageDispatch();
    }
    switchTime = TP_TimerCheck(0);
    switchTime = 60 * 1000 - switchTime;

    ShowSimCardStartUPProcess();

    TP_TimerSet(0, 120 * 1000);
    ...
    TP_ScrGotoxyEx(0, 13);
    TP_LcdPrintf("%s", "Service registering ...");

    while (TP_NetworkGetStatus() == SRVCSTATUS_REGISTERING);

    networkTime = TP_TimerCheck(0);
    networkTime = 120 * 1000 - networkTime;
    ...
    if (TP_NetworkGetStatus() == SRVCSTATUS_REGISTERED)
    {
        ...
        TP_ScrCls();
        TP_ScrGotoxy(3, 0);
        TP_LcdPrintf("Access Point Name:");
        TP_ScrGotoxy(3, 6);
        ...
        TP_ScrGotoxyEx(4, 17);
        TP_SetInputModeControl(KEY_DEMO_CONFIRM, TP_KEY_NONE, KEY_DEMO_POUND);
        TP_SetInputModePosition(59,52);
        TP_SetInputMode(INPUTMODE_LOWCASE);
        TP_ShowInputNum(0, 1, 1);
        memset(apnStr, 0x00, sizeof(apnStr));
        strcpy(apnStr, "cmnet");
        TP_GetHzString(apnStr, 0, 20);

        TP_SetDisplayArea(0, 0, 127, 63);
        TP_ScrCls();
        TP_ScrGotoxy(3, 0);
        TP_LcdPrintf("User Name:");
        TP_ScrGotoxy(3, 6);
        TP_LcdPrintf("Confirm");
        TP_ScrGotoxy(90, 6);
        TP_LcdPrintf("Cancel");
        ...
        TP_SetInputMode(INPUTMODE_LOWCASE);
        TP_ShowInputNum(0, 1, 1);
        memset(userNameStr, 0x00, sizeof(userNameStr));
        TP_GetHzString(userNameStr, 0, 20);

        TP_SetDisplayArea(0, 0, 127, 63);
        TP_ScrCls();
        TP_ScrGotoxy(3, 0);
        TP_LcdPrintf("Password:");
        TP_ScrGotoxy(3, 6);
        TP_LcdPrintf("Confirm");
        TP_ScrGotoxy(90, 6);
        TP_LcdPrintf("Cancel");
        ...
        TP_ShowInputNum(0, 1, 1);
        TP_SetInputMode(INPUTMODE_LOWCASE);
        memset(passwordStr, 0x00, sizeof(passwordStr));
        TP_GetHzString(passwordStr, 0, 20);
        ...
        TP_ScrGotoxyEx(0, 13);
        TP_LcdPrintf("%s", "GPRS login ...");

        TP_TimerSet(0, 120 * 1000);
        TP_WlPPPLogin(apnStr, userNameStr, passwordStr, 0, 0, -1);
        TP_WlPPPLogout();

        gprsTime = TP_TimerCheck(0);
        gprsTime = 120 * 1000 - gprsTime;
    }
    ...
    {
        TP_ScrGotoxyEx(0, 0);
        TP_LcdPrintf("Card switch");
        TP_ScrGotoxyEx(0, 12);
        TP_LcdPrintf("cost: %dms", switchTime);
        TP_Kbflush();
        while (TP_Kbhit() == 0x00)
        {
    ...
    if (networkTime != 0)
    {
        TP_ScrCls();
        TP_ScrGotoxyEx(0, 0);
        TP_LcdPrintf("Service available");
        TP_ScrGotoxyEx(0, 12);
        TP_LcdPrintf("cost: %dms", networkTime);
        TP_Kbflush();
        while (TP_Kbhit() == 0x00)
        {
    ...
    if (gprsTime != 0)
    {
        TP_ScrCls();
        TP_ScrGotoxyEx(0, 0);
        TP_LcdPrintf("GPRS available");
        TP_ScrGotoxyEx(0, 12);
        TP_LcdPrintf("cost: %dms", gprsTime);
        TP_Kbflush();
        while (TP_Kbhit() == 0x00)
        {
    ...
    gprsTime = switchTime + networkTime + gprsTime;
    if (gprsTime != 0)
    {
        TP_ScrCls();
        TP_ScrGotoxyEx(0, 0);
